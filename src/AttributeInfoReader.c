...
void read_attributes(FILE *arquivo_class, JavaClass *class_struct)
{
	...
	for (contador_atributos = 0;
        contador_atributos < class_struct->attributes_count;
        contador_atributos++)
	{
		attribute_types_info(arquivo_class, class_struct,
                        class_struct->attributes + contador_atributos);
	}
}
                      ...
                      AttributeInfo *attribute_struct)
{
	...
	attribute_type = get_utf8_from_cp(class_struct->contant_pool,
                                  attribute_struct->attribute_name_index - 1);
...
void attribute_type_code(FILE *arquivo_class, JavaClass *class_struct, AttributeInfo *attribute_struct)
{
  ...
  printf("ATRI-COUNT%d\n", attribute_struct->code->attribute_count);
...
char* get_utf8_from_cp(CpInfo *cp, u2 info_index)
{
	...
	switch(tag)
	{
		case CONSTANT_Utf8:
			...
			break;
		case CONSTANT_Class:
			string = get_utf8_from_cp(cp, (cp[info_index].Class.name_index)-1);
			break;
