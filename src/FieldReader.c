...
void read_field(FILE *fp, JavaClass* class_file) {
  ...
  for (i = 0; i < fields_count; i++) {
    ...
    class_file->fields[i].attributes = (CvInfo *) malloc(sizeof(CvInfo) * class_file->fields[i].atributes_count);
    ...
    for (j = 0; j < class_file->fields[i].atributes_count; j++) {
      ...
      class_file->fields[i].attributes->constant_value_index = read_2_bytes(fp);
    }
  }
}
